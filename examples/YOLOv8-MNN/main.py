...
class YOLOv8:
    ...
    def __init__(self, mnn_model, input_image, confidence_thres, iou_thres, data_cfg='ultralytics/cfg/datasets/coco.yaml'):
        """
    ...
    def main(self):
        """
        Performs inference using an MNN model and returns the output image with drawn detections.

        Returns:
            output_img: The output image with drawn detections.
        """
        # Create an inference session using the MNN model and specify execution providers
        session = Network()
        session.load_model(self.mnn_model)
        # session = ort.InferenceSession(self.mnn_model, providers=["CUDAExecutionProvider", "CPUExecutionProvider"])
        ...
        self._output_names = "output0"
        # Preprocess the image data
        img_data = self.preprocess()
        
        outputs = session.inference((self.input_width, self.input_height), self._output_names, img_data)
...
if __name__ == "__main__":
    ...
    detection = YOLOv8(args.model, args.img, args.conf_thres, args.iou_thres, data_cfg=args.data_cfg)
